...
[f_death,S,mu] = Exp_smooth_trend_mul(total_death_s,date_s1, alpha, beta,np,nf);
% [f_death_fit,delta] = polyval(p,datenum(date_s1),S,mu);
var_forecast_death = prediction_int_mul(total_death_s',f_death, alpha,beta,np,nf);
...
[f_cases,S2,mu2] = Exp_smooth_trend_mul(total_cases_s,date_s2, alpha2, beta2,np,nf);
var_forecast_cases = prediction_int_mul(total_cases_s',f_cases, alpha2,beta2,np,nf);
...
legend('training data','real data','forecast','Location','NorthWest','Fontsize',16)
% ylim([0,y11(end)])
ylim([0,10+f_death(end)])
xlim([date_s1(1),f_date_s1(end)])
% set (gca, 'YScale' , 'log' )
filename1 = ['Deaths2_',country_name,'.png'];
...
legend('training data','real data','forecast','Location','NorthWest','Fontsize',16)
% ylim([0,y12(end)])
ylim([0, 10+f_cases(end)])
xlim([date_s2(1),f_date_s2(end)])
% set (gca, 'YScale' , 'log' )
filename2 = ['Cases2_',country_name,'.png'];
